<x-admin-layout title="Edit Shift">
    <div class="max-w-2xl space-y-6">
        <h1 class="text-2xl font-semibold text-slate-800 dark:text-slate-100">Edit Shift</h1>

        <form action="{{ route('admin.shifts.update', $shift) }}" method="POST" class="space-y-4 rounded-xl bg-white dark:bg-slate-800 p-6 border border-slate-200 dark:border-slate-700">
            @csrf
            @method('PATCH')
            <div>
                <label for="opened_at" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Opened At</label>
                <input type="datetime-local" name="opened_at" id="opened_at" value="{{ old('opened_at', $shift->opened_at?->format('Y-m-d\TH:i')) }}" required
            ...
            <div>
                ...
                <input type="datetime-local" name="closed_at" id="closed_at" value="{{ old('closed_at', $shift->closed_at?->format('Y-m-d\TH:i')) }}"
                       class="w-full rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white focus:ring-primary focus:border-primary">
